  ...
  ({ command, query }, ref) => {
    ...
    if (allItems.length === 0) {
      return (
        <div
          ref={menuRef}
          className="z-50 rounded-lg border border-border bg-card shadow-lg p-3 text-sm text-muted-foreground"
        >
          No results
        </div>
      );
    ...
    return (
      <div
        ref={menuRef}
        className="z-50 w-72 max-h-80 overflow-y-auto rounded-lg border border-border bg-card shadow-lg py-1"
      >
        {groups.map((group) => (
          <div key={group.label}>
